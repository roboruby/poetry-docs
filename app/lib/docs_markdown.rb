  ...
  class << self
    ...
    def i18n(entry)
      <<~MD
        ...
        Poetry splits translation duty: the library's interface strings (accessible names like Close and Clear selection, empty and loading states, screen-reader counts) resolve through I18n under `poetry.<component>.<key>`, shipped in poetry-ui's `config/locales/en.yml` - the complete key list, loaded by engine convention, with standard pluralization and `%{}` interpolation. Host locale files load after the engine's, so redefining a key overrides it per key; add a language by shipping the `poetry.*` keys for that locale, and set `config.i18n.fallbacks = [:en]` so a key you have not translated yet reads the shipped English instead of raising. Visible copy is never a library key: `label:`, `placeholder:`, `empty_text:`, `filter_label:`, and `hint:` are options with English zero-config defaults (DatePicker's "Pick a date") - pass `t(".key")` translations at call sites. The FormBuilder derives labels from `human_attribute_name` (the `activerecord.attributes.*` keys you already maintain) and validation messages ride Active Model's own I18n chain; apps migrating with poetry-simple_form keep their existing `simple_form.*` label, hint, and placeholder keys working as a fallback chain. DateField and TimeField segment names and placeholders live under `poetry.date_field.*`; Calendar's month dropdown (`caption_layout: :dropdown`) reads Rails' own `date.month_names` keys.
      MD
    end
